<template>
  ...
  <b-row>
     <b-col cols="12" xl="12">
        <b-button pill variant="success" class="mb-10 float-right" to='/dashboard/properties'>
  ...
  <b-row>
    <b-col cols="12" xl="12">
      ...
      <b-table :key="tableKey" :hover="hover" :busy="isBusy" :striped="striped" :bordered="bordered" :small="small" :fixed="fixed" responsive="sm" :items="items" :fields="fields" :current-page="currentPage" :per-page="perPage" >
          ...
          <template #cell(contracted)="data">
            <b-badge :variant="getBadge(data.item.contracted)">{{data.item.contracted? 'contratado' : 'não contratado'}}</b-badge>
          </template>
          <template #cell(action)="row" >
            <b-button pill variant="success" title="Restaurar" class="ml-1" size="sm" @click="info(row.item, $event.target)">
    ...
    <b-modal :id="deleteModal.id" centered header-bg-variant="success"
        :title="deleteModal.title" ok-only ok-variant="success"
        @ok="restoreItem(deleteModal.content)" @hide="resetDeleteModal">
         <ul class="list-group">
...
export default {
  ...
  methods: {
    ...
    restoreItem(item){
        const url = `${apiRoutes.services.properties}/restore/${item.id}`;
    ...
    info(item, button) {
      this.deleteModal.title = `Restaurar ${item.title}?`
    ...
    getProperties(){
        let url = `${apiRoutes.services.properties}/trashed?page=${this.currentPage}`;
        if (this.search !== '') {
